...
def check_color(color_dict, stabilizer):
    ...
    for x in color_dict.keys():
        ...
        if any(
            set(stabilizer) == set(stabilizer_generator)
            for stabilizer_generator in color_dict[x]
        ):
            assert found != True
            color = x
            found = True
...
def createStimBaselineandSOA(
    ...
    x_s, z_s, cx_arr, _ = parse_to_tuple(stabilizer_matrix, n)
    # print("x's", x_s)
    # print("z's", z_s)
    data = []
    ancilla = []
    ...
    if not (
        surface_code
    ):  # do some preprocessing, COLOR DICT LOOKS COMPLETELY DIFFERENT IN THE BASELINE/SOA VERSION
        color_dict = {}
        for i in range(3):
            color_dict[i] = []
    ...
    with open(file_path, "w") as file:
        ...
        file.write(
            "X_ERROR(" + str(errorX) + ") " + data_string + ancilla_string + "\n"
        )
        tabString += (
            "  X_ERROR(" + str(errorX) + ") " + data_string + ancilla_string + "\n"
        )
        file.write(
            "Y_ERROR(" + str(errorY) + ") " + data_string + ancilla_string + "\n"
        )
        tabString += (
            "  Y_ERROR(" + str(errorY) + ") " + data_string + ancilla_string + "\n"
        )
        file.write(
            "Z_ERROR(" + str(errorZ) + ") " + data_string + ancilla_string + "\n"
        )
        tabString += (
            "  Z_ERROR(" + str(errorZ) + ") " + data_string + ancilla_string + "\n"
        )
        ...
        for layer in cx_lists:
                ...
                if len(building_cx_x[pos]) > 0:
                    for stabilizer_generator in copy_CX_x:
                        if set(stabilizer_generator) == set(building_cx_x[pos]):
                            ...
                            building_cx_x[
                                pos
                            ] = []  # for ancilla reuse case we need to reset it to nothing
            ...
            if (
                len(measurement_xs) > 0
            ):  # assumes we CAN finish an X and a Z in a parallel step
                H_string = "H "
                H_string += (
                    measure_ancilla_string_x + " "
                )  # assumes never measuring an x and a z stabilizer in same step
                file.write(H_string + "\n")
                tabString += "  " + H_string + "\n"
                ###QUEUE a  measurement and reset of the ancilla here
                file.write("X_ERROR(" + str(p) + ") " + measure_ancilla_string_x + "\n")
                tabString += (
                    "  " + "X_ERROR(" + str(p) + ") " + measure_ancilla_string_x + "\n"
                )
                ...
                depolarize_after_h_string = (
                    "DEPOLARIZE1(" + str(depolarize_after_h) + ") "
                )
                for i in ancilla:
            ...
            if len(measurement_zs) > 0:
                ...
                tabString += (
                    "  " + "X_ERROR(" + str(p) + ") " + measure_ancilla_string_z + "\n"
                )
        ...
        if surface_code:
            ...
            for i in range(len(master_measurement)):
                if master_measurement[i][0] == "Z":
                    ...
                    file.write(
                        "DETECTOR(0,0,0,"
                        + str(check_color(color_dict, master_measurement[i][2]))
                        + ") "
                        + "rec["
                        + str(number)
                        + "]\n"
                ...
                if len(building_cx_x[pos]) > 0:
                    for stabilizer_generator in copy_CX_x:
                        if set(stabilizer_generator) == set(building_cx_x[pos]):
                            ...
                            measure_ancilla_string_x += str(pos)
                            measure_ancilla_string_x += " "
                            back_master_measurement.append(
                                ("X", pos, building_cx_x[pos])
                            )
                            building_cx_x[
                                pos
                            ] = []  # for ancilla reuse case we need to reset it to nothing
        ...
        if surface_code:
            for i in range(len(back_master_measurement)):
                ...
                for s in range(len(master_measurement)):
                    if master_measurement[s][2] == stabilizer[::-1]:
                        second = (
                            -len(back_master_measurement) - len(master_measurement) + s
                        )
            ...
            for i in range(len(back_master_measurement)):
                ...
                for s in range(len(master_measurement)):
                    if (
                        master_measurement[s][2] == stabilizer[::-1]
                        and back_master_measurement[i][0] == master_measurement[s][0]
                    ):  # check types match as well because color codes have 2 stabilizers that fit this
                        second = (
                            -len(back_master_measurement) - len(master_measurement) + s
                        )
                # if (i + n % 2 in ancilla): #because each ancilla is assigned
                ...
                if back_master_measurement[i][0] == "Z":
                    backString += (
                        "  DETECTOR(0,0,0,"
                        + str(check_color(color_dict, back_master_measurement[i][2]))
                        + ") "
                        + "rec["
                        + str(number)
                        + "] "
                    ...
                    backString += (
                        "  DETECTOR(0,0,0,"
                        + str(
                            (check_color(color_dict, back_master_measurement[i][2])) + 3
                        )
                        + ") "
                        + "rec["
                        + str(number)
        ...
        if surface_code:
            for i in range(len(master_measurement)):
                ...
                for s in range(len(back_master_measurement)):
                    if back_master_measurement[s][2] == stabilizer[::-1]:
                        second = (
                            -len(master_measurement) - len(back_master_measurement) + s
                        )
            ...
            for i in range(len(master_measurement)):
                ...
                for s in range(len(back_master_measurement)):
                    if (
                        back_master_measurement[s][2] == stabilizer[::-1]
                        and master_measurement[i][0] == back_master_measurement[s][0]
                    ):
                        second = (
                            -len(master_measurement) - len(back_master_measurement) + s
                        )
                ...
                if master_measurement[i][0] == "Z":
                    ...
                    file.write(
                        "  DETECTOR(0,0,0,"
                        + str((check_color(color_dict, master_measurement[i][2])) + 3)
                        + ") "
                        + "rec["
                        + str(number)
                        + "] "
        ...
        for i in range(len(back_master_measurement)):
            ...
            if x[0] == "Z":
                ...
                if surface_code:
                    ...
                    detector_string = (
                        "DETECTOR(0,0,0," + str(check_color(color_dict, x[2])) + ") "
                    )
                    for d in data_list:
                        detector_string += "rec[" + str(negative_dict[d]) + "] "
                    ancilla_measurement = -len(back_master_measurement) - len(data) + i
        ...
        for pphys in pphys_range:
            for prop in ancilla_proportion:
                ...
                for x in range(len(all_timings)):
                    ...
                    stabilizer_string = (
                        "stabilizer_metadata/" + short_name + "matrix.pkl"
                    )
                    ...
                    if (
                        ...
                        or number == "61"
                        or number == "91"
                    ):  # quick and easy way to determine color codes
                        surface_code = False
                    else:
                        surface_code = True
                    ...
                    if surface_code:
                        ...
                        cx_lists, timings, file_baseline_mode = read_pickled_info(
                            c, t
                        )  # file_baseline_mode deprecated
                        # ADDING SYMPATHETIC COOLING 9/23/2024
                        # print("timings for baseline", timings)
                        for i in range(
                            len(timings)
                        ):  # BASELINE METHOD FOR ADDING TIMINGS ADDED 11/19/24
                            if (
                                timings[i] != 100
                            ):  # then it must be a shuttling op (when ion chain lengths are bounded at 5, 100 is fine. If doing sensitivity analysis it must change according to trap scaling to find gate time and filter it out)
                                timings[i] += 473

                        # print("timings after sympathetic cooling on each merge", timings)

                        createStimBaselineandSOA(
                            full_name,
                            cx_lists,
                            timings,
                            pphys,
                        ...
                        colors_string = (
                            "stabilizer_metadata/" + short_name + "colors.pkl"
                        )
                        ...
                        for i in range(
                            ...
                            if (
                                timings[i] != 100
                            ):  # then it must be a shuttling op (same thing with gate times as noted above, if ion trap capacity increases, need to adjust filter for gate times accordingly)
                                timings[i] += 473
