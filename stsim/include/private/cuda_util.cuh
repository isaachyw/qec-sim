...
// CLUSTER BASED
#define PGAS_P(arr, i, val)                                                    \
  nvshmem_double_p(&(arr)[(i) & ((m_gpu) - 1)], (val), ((i) >> (lg2_m_gpu)))
#define PGAS_G(arr, i)                                                         \
  nvshmem_double_g(&(arr)[(i) & ((m_gpu) - 1)], ((i) >> (lg2_m_gpu)))
#define BARR_NVSHMEM                                                           \
...
#define SAFE_ALOC_HOST_CUDA(X, Y)                                              \
  cudaSafeCall(cudaMallocHost((void **)&(X), (Y)));
...
__device__ inline uint64_t swapBits_cu(uint64_t n, uint64_t p1, uint64_t p2) {
...
__global__ void outerProduct(double *matrix_real, double *matrix_imag,
                             double *v1_real, double *v1_imag, double *v2_real,
                             double *v2_imag, size_t size_1, size_t size_2) {
  // Is this fast? no. Could it be more optimized? yes. Does it get the job done
  // for now? probably.
  ...
  for (size_t i = tid; i < size_1 * size_2; i += n_threads) {
    ...
    double real_coeff =
        v1_real[idx1] * v2_real[idx2] + v1_imag[idx1] * v2_imag[idx2];
    double imag_coeff =
        v1_imag[idx1] * v2_real[idx2] - v1_real[idx1] * v2_imag[idx2];
...
__device__ bool hasEvenParity(unsigned long long x, const size_t *in_bits,
                              const size_t in_bits_size) {
  size_t count = 0;
...
__device__ bool hasEvenParity_cu(unsigned long long x,
                                 const size_t in_bits_size) {
  size_t count = 0;
  for (size_t i = 0; i < in_bits_size; ++i) {
    if (x & (1ULL << i)) {
...
__device__ double parity(unsigned long long num) {
  ...
  num ^= num >> 1;
  return num &
         1; // Return the last bit, which is the parity of the original number
}

__global__ void gpu_exp_z(const double *sv_real, const double *sv_imag,
                          double *result, const unsigned long long dim,
                          const int offset) {
  ...
  if (index < dim) {
    ...
    double magnitude_sq =
        sv_real[index] * sv_real[index] + sv_imag[index] * sv_imag[index];
...
__global__ void gpu_exp_z_bits(const size_t *in_bits, size_t in_bits_size,
                               const double *sv_real, const double *sv_imag,
                               double *result, const unsigned long long dim,
                               const int offset) {
  extern __shared__ double sdata[]; // Shared memory to store partial sums
  int tid = threadIdx.x;
