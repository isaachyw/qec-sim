...
public:
  ...
  STAB_CPU(IdxType _n_qubits) : QuantumState(SimType::STAB) {
    ...
    x.resize(
        rows,
        std::vector<int>(
            cols, 0)); // first 2n+1 x n block. first n represents destabilizers
                       // second n represents stabilizers + 1 extra row
    z.resize(rows, std::vector<int>(cols, 0)); // second 2n+1 x n block to form
                                               // the 2n+1 x 2n sized tableau
  ...
  IdxType get_qubits() override { return n; }
  ...
  int get_num_rows() override { return rows; }
  ...
  void reset_state() override {
    ...
    if (rz_flag)
      std::fill(rz_r.begin(), rz_r.end(), 0);
    // The 2n+1 th row is scratch space
  ...
  std::vector<int32_t> get_measurement_results() override { return m_results; }

  // Get the stabilizers in the tableau, i.e. all of the Pauli strings that
  ...
  // {
  //     x_erasure.resize(rows, std::vector<int>(cols,0)); //first 2n+1 x n
  //     block. first n represents destabilizers
  //                                                //second n represents
  //                                                stabilizers + 1 extra row
  //     z_erasure.resize(rows, std::vector<int>(cols,0)); //second 2n+1 x n
  //     block to form the 2n+1 x 2n sized tableau r_erasure.resize(rows, 0);
  //     //column on the right with 2n+1 rows
  //     //The 2n+1 th row is scratch space
  ...
  ComplexMatrix get_density_matrix() override {
    ...
    for (const auto &line : full_paulis) {
      ...
      for (char p : pauli_string) {
        if (p == 'I')
          S = kroneckerProduct(S, pauliI()).eval();
        else if (p == 'X')
          S = kroneckerProduct(S, pauliX()).eval();
        else if (p == 'Y')
          S = kroneckerProduct(S, pauliY()).eval();
        else if (p == 'Z')
          S = kroneckerProduct(S, pauliZ()).eval();
        else
          throw std::logic_error("Invalid Pauli character in stabilizer");
      }
  ...
  // Returns a map of stabilizers and the number of times they ocurr in the
  // tableau
  void stabilizer_count(std::unordered_map<std::string, std::pair<int, int>>
                            &stab_counts) override {
    for (int i = 0; i < rows; i++) {
    ...
    // Go back through and remove excess stabilizers, then add any odd number
    // back
        ...
        for (int j = 0; j < rows; j++) {
          ...
          if (rows > 0) {
            if (get_stabilizer_line(j).first == stabilizer) {
              // std::cout << "Removed: " << stabilizer << " " <<
              // get_stabilizer_line(j).second << std::endl;
              remove_stabilizer(j);
              j--;
            }
          }
  ...
  // If one of the stabilizers doesn't commute with the provide pauli string,
  // return false
  ...
  void add_stabilizer_bits(std::vector<int> new_x, std::vector<int> new_z,
                           int phase_bit = 0) override {
    stabCounts++;
    rows++;
    x.push_back(new_x);
    z.push_back(new_z);
  ...
  // Replaces a stabilizer pauli string at some row in the Tableau. Useful for
  // initializing a new Tableau in a for loop without circuit initialization
  void replace_stabilizer(std::string pauliString, int stabPos) override {
    ...
    // Stabilizer and destabilizer addition
    for (int i = 0; i < pauliString.length(); i++) {
      // paulistring length is the same as number of qubits, so we use it as the
      // column iterator
      switch (pauliString[i]) {
      case 'I':
        // stab
  ...
  //  void tempRowsum(int h, int i, std::vector<std::vector<int>>& temp_x,
  //  std::vector<std::vector<int>>& temp_z, std::vector<int>& temp_r)
  //  {
  //      int sum = 0;
  //      for(int j = 0; j < n; j++)
  //      {
  ...
  // //Provides a bit/shot measurement output without affecting the original
  // tableau IdxType *measure_all(IdxType shots = 2048) override
  // {
  //     //Each index of shotResults is a possible result from a full
  //     measurement, ex. 01100101 in an 8 qubit system
  //     //The integer at that position is the number of times that result
  //     occured if (totalResults != nullptr) {
  //         SAFE_FREE_HOST(totalResults);
  //     }
  ...
  //     {
  //         //Make a copy of the class being measured so many shots can be
  //         performed std::vector<std::vector<int>> temp_x = x;
  ...
  //             {
  //                 //std::cout << "x at [" << p_index << "][" << a << "] = "
  //                 << x[p_index][a] << std::endl; if(temp_x[p_index][a])
  //                 {
  //                     p = p_index;
  //                     break;
  //                 }
  ...
  //                 // std::cout << "Random measurement at qubit " << a << "
  //                 value: " << (temp_r[p] << a) << std::endl;
  //             }
  //             else
  //             {
  ...
  //                     if(temp_x[i][a] == 1)
  //                     {
  //                         //std::cout << "Perform rowsum at " << i << " + n"
  //                         << std::endl; tempRowsum(rows-1, i+half_rows,
  //                         temp_x, temp_z, temp_r);
  //                     }
  //                 }

  //                 // std::cout << "Deterministc measurement at qubit " << a
  //                 << " value: " << (temp_r[rows-1] << a) << std::endl;
  ...
  // //Provides a bit/shot measurement output without affecting the original
  // tableau IdxType **measure_all_long(IdxType shots = 2048) override
  // {
  //     //Each index of shotResults is a possible result from a full
  //     measurement, ex. 01100101 in an 8 qubit system
  ...
  //     {
  //         //Make a copy of the class being measured so many shots can be
  //         performed temp_x = x; temp_z = z; temp_r = r; for(int a = 0; a < n;
  //         a++)
  //         {
  //             int p = -1;
  //             for(int p_index = half_rows; p_index < rows-1; p_index++)
  ...
  //                 totalResultsLong[shot][a/64] |=  (temp_r[rows-1] <<
  //                 (a%64));
  //                 // std::cout << "Random measurement at qubit " << a << "
  //                 value: " << (temp_r[p] << a) << std::endl;
  //             }
  ...
  void sim(std::shared_ptr<NWQSim::Circuit> circuit,
    ...
    assert((circuit->num_qubits() == n) &&
           "Circuit does not match the qubit number of the state!");
    ...
    if (Config::PRINT_SIM_TRACE) {
      printf("\n============== STAB-Sim ===============\n");
      printf("n_qubits:%lld, n_gates:%lld, ncpus:%d, comp:%.3lf ms, comm:%.3lf "
             "ms, sim:%.3lf ms\n",
             n, n_gates, 1, sim_time, 0., sim_time);
      printf("=====================================\n");
    }
    //=========================================
  }
  ...
  std::vector<std::vector<int>> get_graph_matrix() override {
    ...
    if (x.size() != z.size()) {
      std::cerr << "Error: Matrices must have the same number of rows to "
                   "append column-wise."
                << std::endl;
    }
    ...
    std::cout << "----- X rank after reduction ----- " << xRank
              << ", stabCounts: " << stabCounts << std::endl;
    if (xRank != stabCounts) {
      zReduction(graphMatrix);
      // permute qubits using the qubitIndex

      // Adjust qubit index to maximize the size of the identity block in the Z
      // part
    ...
    */
    ...
    std::vector<std::vector<int>> D(graphMatrix.size() - xRank,
                                    std::vector<int>(xRank));
    std::vector<std::vector<int>> I2(graphMatrix.size() - xRank,
                                     std::vector<int>(n - xRank - 1));
    std::vector<std::vector<int>> sign2(graphMatrix.size() - xRank,
                                        std::vector<int>(1, 0));

    /*
    If the stabilizer is full rank (stabilizer count = qubit number), the
    stabilizer tableau can be reduced to
    [[I A | B 0]
     [0 0 | D I]]
    ...
    */
    ...
    std::vector<std::vector<int>> newGraph(
        graphMatrix.size(), std::vector<int>(graphMatrix[0].size(), 0));
...
protected:
  ...
  int countPivots(std::vector<std::vector<int>> &matrix) {
    ...
    for (int col = 0; col < cols && leadCol < rows; col++) {
      ...
      // Swap the row with the leading 1 to the top of the current column's
      // range
      ...
      for (int row = leadCol + 1; row < rows; row++) {
        if (matrix[row][col] == 1) {
          // XOR the current row with the row containing the leading 1 to
          // eliminate the 1
          for (int j = 0; j < cols; j++) {
  ...
  std::vector<int>
  searchNonZeroRows(const std::vector<std::vector<int>> &matrix, int col,
  ...
  extractHalf(const std::vector<std::vector<int>> &matrix, bool left) {
  ...
  void gaussianElimination(std::vector<std::vector<int>> &matrix) {
    ...
    for (int i = 0; i < matrix.size(); i++) {
      ...
      if (allzero) {
        matrix =
            std::vector<std::vector<int>>(matrix.begin(), matrix.begin() + i);
        break;
      }
    }
  }
  ...
  void reduceUp(std::vector<std::vector<int>> &matrix,
                const std::vector<int> &columnIndices) {
    int j0 =
        columnIndices[columnIndices.size() - 1]; // The index of the last row in
                                                 // columnIndices for addition
  ...
  void reduceDown(std::vector<std::vector<int>> &matrix,
                  const std::vector<int> &columnIndices) {
  ...
  // Function to perform a row insertion (row permutation) operation in the
  // tableau
  void rowInsert(std::vector<std::vector<int>> &matrix, int i, int j) {
    ...
    if (i < j) {
      throw std::invalid_argument(
          "The inserted row index is before the insertion place.");
    }
    if (i == j) {
    ...
    matrix =
        permutedMatrix; // Update the original matrix with the permuted matrix
  }

  void qubitReorder(std::vector<std::vector<int>> &matrix,
                    std::vector<int> indices) {
    // Check if the length of the indices matches the number of qubits
    if (indices.size() != n) {
      throw std::invalid_argument(
          "The permutation list does not match the qubit number.");
    }

    // Create a new vector of indices to map old qubit positions to new
    // positions
    ...
    std::vector<std::vector<int>> permutedTableau(
  ...
  void reduction(std::vector<std::vector<int>> &graphMatrix) {
    ...
    for (int col = 0; col < n * 2; col++) {
      ...
      if (ones.empty()) {
        ...
        std::vector<int> onesUp =
            searchNonZeroRows(graphMatrix, col, 0, rowPointer);
  ...
  int damping_generator(double p,
                        double gamma) { // Remove negative and renormalize
    // Range [0.0, 1.0)
    ...
    //     std::cerr << "Assertion failed: c0 + c1 + c2 = " << total << "
    //     (expected ~1)\n"; std::cerr << "p = " << p << ", gamma = " << gamma
    ...
    //     std::cerr << "Assertion failed: c0_tot + c1_tot + c2_tot = " <<
    //     tot_total << " (expected ~1)\n"; std::cerr << "p = " << p << ", gamma
    //     = " << gamma << std::endl; assert(false);
    // }
    // if(c0_tot < 0){
    ...
    std::cout << "c~: " << c0_tot << " " << c1_tot << " " << c2_tot
              << std::endl;
  ...
  void reset_routine(int a) {
    ...
    if (p > -1) {
      ...
      temp_result = r[p];
      // std::cout << "Random measurement at qubit " << a << " value: " << (r[p]
      // << a) << std::endl;
    }
    // Deterministic
    else {
      ...
      // std::cout << "Deterministc measurement at qubit " << a << " value: " <<
      // (r[rows-1] << a) << std::endl;
  ...
  void simulation_kernel(std::vector<Gate> &gates) {
    ...
    for (int k = 0; k < g; k++) {
      ...
      case OP::CX: {
        ...
        //         for (int i = 0; i < rows - 1; i++)
        //         {
        //             r[i] ^= (x[i][ctrl] & z[i][target] & (x[i][target] ^
        //             z[i][ctrl] ^ 1)); x[i][target] ^= x[i][ctrl]; z[i][ctrl]
        //             ^= z[i][target];
        //         }
        //     };
      ...
      case OP::CHAN1: {
        ...
        if (probs.size() < 3) {
          std::cerr << "CHAN1: missing probabilities, size=" << probs.size()
                    << std::endl;
          break;
        }
      ...
      case OP::RX:
        ...
        // H S
          ...
          std::cout << "Non-Clifford angle in RX gate! "
                    << OP_NAMES[gate.op_name] << "(" << gate.theta << ")"
                    << std::endl;
      ...
      case OP::RZ:
        ...
        // }
          ...
          std::cout
              << "Non-Clifford angle in RZ gate! Arbitrary RZ already used: "
      ...
      case OP::RY:
        ...
        // X H
          ...
          std::cout << "Non-Clifford angle in RY gate! "
                    << OP_NAMES[gate.op_name] << "(" << gate.phi << ")"
                    << std::endl;
          std::logic_error("Invalid gate type");
          exit(1);
        }
      ...
      case OP::M: {
        ...
        if (p > -1) {
          ...
          // std::cout << "Random measurement " << measurement_count << ": " <<
          // randomBit << std::endl;
        ...
        // Deterministic
          ...
          if (rz_flag && (r[rows - 1] != rz_r[rows - 1])) {
            ...
            // std::cout << "Determ cos2: " << pow(rz_coeff.first,2) <<
            // std::endl;
      ...
      case OP::RESET: {
        ...
        // Deterministic
          ...
          if (rz_flag &&
              (r[rows - 1] !=
               rz_r[rows - 1])) // Case: branches disagree on measurement
                                // outcome. Collapse the branching.
          {
            double random = prng_uniform01(seed, measurement_count);
            std::cout << "Determ rand: " << random << std::endl;
            ...
            if (r[rows - 1] == 1) // Apply X to the updated r column
            {
              for (int i = 0; i < rows - 1; i++) {
                r[i] ^= z[i][a];
                     ...
                     (r[rows - 1] == 1)) // Case: both branches agree on one.
                                         // Apply X to both to flip back to 0.
          {
            for (int i = 0; i < rows - 1; i++) {
              r[i] ^= z[i][a];
              rz_r[i] ^= z[i][a];
                     ...
                     1) // Case: no branching, just flip r if it resulted in 1.
          {
            for (int i = 0; i < rows - 1; i++) {
              r[i] ^= z[i][a];
            }
          } else // Case: r resulted in 0, no flip needed (if rz_r disagreed the
                 // first case would handle it).
            continue;
        }
        break;
      }
